...
for shape in shapes:
    ...
    suptitle = 'Quadrilateral cells' if shape == 'quads' else 'Triangular cells'
    plotname = 'fig_7_galewsky_quads' if shape == 'quads' else 'fig_8_galewsky_tris'
    ...
    results_dirnames = [base_results_dirname+'_'+option for option in results_options]
    ...
    for i, (ax, dirname, title, xticklabels, xlabel, xticks) in \
        enumerate(zip(axarray, results_dirnames, titles, [None,None,[-270,90]],
                      [None,None,r'$\lambda \ / $ deg'], [None,None,[-3*np.pi/2,np.pi/2]])):
