...
plot_times = 'all'
base_plotname = 'results/pdc_idealised_paper/figures/quick_final'
...
init_time_idx = 0
time_idx = -1
field_names = ['exner_pert','theta','theta_pert','theta_e','theta_e_pert','m_v','m_cl','m_v_pert','m_cl_pert']
...
init_data_file = Dataset('results/'+results_dirname+'/lfric_initial.nc','r')
...
for field_name in field_names:
    ...
    if field_name == 'theta_pert':
        ...
        for i in range(len(m_v_data[0,:])):
            field_data[:,i] = m_v_data[:,i] - init_data[:,0]
        ...
        for i in range(len(m_cl_data[0,:])):
            field_data[:,i] = m_cl_data[:,i] - init_data[:,0]
        ...
        for i in range(len(exner_data[0,:])):
            field_data[:,i] = exner_data[:,i] - init_data[:,0]
